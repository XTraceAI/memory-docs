...
// inject `prompt`; `memories` is the deduped, score-ranked union; `scopes` is per-scope counts
```

**Params**: `query` (required), plus at least one of `user_id` / `group_ids`; optional `agent_id`, `app_id`, `mode` (default `compose`), `limit` (default 10).
