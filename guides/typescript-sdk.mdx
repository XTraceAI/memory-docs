...
**Required body fields**: `messages`, `user_id`, `conv_id`. Optional: `agent_id`, `app_id`, `group_ids` (tag the extracted memories to [groups](/guides/groups)), `timestamp_format`, `extract_artifacts` (defaults to `true` — pass `false` to skip the artifact-extraction stage).
...
See [Searching memories](/guides/searching-memories) for scoping, modes, and groups.
...
Axes **AND** within a pool; pools **OR**. So `[{ user_id }, { app_id: 'product-kb' }]` reads "alice's memories OR the product KB," while `{ user_id, app_id }` in *one* pool would AND them.
...
Register and manage **groups** — shared tagging targets. Register a group, then pass its id in `ingest({ group_ids })` and read it back with `search`/`recall`. See the [Groups guide](/guides/groups).
...
|---|---|---|
| `create({ name, prompt? })` | `Group` | `prompt` tells the ingest classifier what belongs to this group; omit it for a [catch-all](/guides/groups#the-model) that receives every shareable memory |
...
- **[Ingesting memories](/guides/ingesting-memories)** — async/sync, polling, group tagging
- **[Searching memories](/guides/searching-memories)** — scoping, modes, `recall`
