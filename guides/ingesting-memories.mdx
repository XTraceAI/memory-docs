...
Optional: `agent_id`, `app_id`, `metadata` (arbitrary key/value, becomes filterable on search), `extract_artifacts` (defaults to `true` — pass `false` to skip the artifact-extraction stage, the most expensive part of the pipeline).
