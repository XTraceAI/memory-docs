...
- **Best-effort.** Delivery has a few quick retries, but an event may still be dropped or duplicated — dedupe on `job_id` and treat the job-polling endpoint as your fallback (see [Delivery semantics](#delivery-semantics)).
...
```json Response
{
  ...
  "enabled": true,
  "secret": "whsec_<your-signing-secret-shown-once>",
...
<Warning>
The full `secret` is returned **only here** — on first create, or when you rotate it. Store it now. Every later `GET` returns it masked (`whsec_••••wxyz`). You need the full value to verify signatures.
</Warning>
...
Delivery is **best-effort** — a delivery may be **dropped or duplicated**, so it's neither strictly at-most-once nor at-least-once:
...
- **Duplicates are possible.** If your handler processes the event but its `2xx` is lost (or times out) before XTrace records it, the retry delivers the same event again. **Dedupe on `job_id`.**
- **Drops are possible.** If every retry fails, the event is given up on. Deliveries are independent of the ingest job — a dropped webhook never changes the job's outcome (the memories are already stored).
- **Your fallback is the job endpoint.** `GET /v1/memories/jobs/{job_id}` always returns the terminal result. For anything you can't afford to miss, reconcile against it rather than relying on the webhook alone.
...
**Make your handler idempotent.** Retries mean the same event can arrive more than once — key on `job_id` so a re-delivery doesn't double-process.
