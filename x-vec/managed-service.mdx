...
description: XTraceIntegration is the single entry point for the XTrace API — security model, connecting, loading, querying, metadata search, chunk operations, and execution context management.
---
...
| Paillier private key | Never transmitted in plaintext. See [Execution context](#execution-context) below. |

**What XTrace can see**

| Item | Notes |
|---|---|
| Metadata tags (`tag1`–`tag5`, `facets`) | Stored and indexed in plaintext. See [Metadata filtering](/x-vec/metadata-filtering) for mitigation strategies. |
...
The `ExecutionContext` bundles the Paillier key pair and the AES key under a single protected object. A **key provider** controls how the AES key is generated and protected.
...
Neither your passphrase nor your KMS plaintext DEK are ever transmitted. Without the corresponding key provider, the stored blob cannot be decrypted — XTrace cannot recover the secret key and cannot decrypt your chunk content or vectors.

See [Configuration](/x-vec/configuration) for the full key provider reference.
...
See [Metadata filtering](/x-vec/metadata-filtering) for the full filter syntax, operator reference, and performance guidance.
...
- Metadata fields use the fixed schema: `tag1`–`tag5` and `facets`. See [Metadata filtering](/x-vec/metadata-filtering) for field semantics and operator reference.
